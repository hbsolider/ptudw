...
module.exports={
    ...
    getBtoS: id => db.load(`select users.*,a.nameProduct,notify.* from users, notify,product a where users.id = notify.idbidder and notify.idseller = ${id} and notify.status = 0 and a.id = notify.idproduct`),
    //get from seller to bidder
    getStoB: id => db.load(`select users.*,a.nameProduct,notify.* from users, notify,product a where users.id = notify.idseller and notify.idbidder = ${id} and notify.status != 0 and a.id = notify.idproduct`),
